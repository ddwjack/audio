...
/*--------------------------选择上传截图的JS部份开始-------------------------------------------------------------------------------*/
window.ShearPhoto.frameUpImg = function(configs) {
     console.log(configs)
          ...
          typeof configs.erro !== "function" && (configs.erro = function() {});
          this.FORM.action = configs["url"];
          var this_ = this;
...
window.ShearPhoto.frameUpImg.prototype = {
          ...
          DelCreateUpfile:function() {
               ...
               this.upfile.onchange = this.upfile.onclick= this.upfile.blur = null;
          ...
          Upsubmit:function(upfile) {
               ...
               // alert(1921)
                    try {
                              ...
                              if (this.inArray(filestype.toLowerCase(), this.configs.UpType) == -1) {
                                        this.DelCreateUpfile();
                                        alert("请选择正确图片类型");
                                        return;
                              }
                              ...
                              if (files) {
                                        files = files[0];
										files.type && (type= files.type==="image/gif" ? "image/jpeg":files.type,this.configs.HTML5.ImagesType = type); 
										if (files.size <= 0) {
                                                  this.DelCreateUpfile();
                                                  this.configs.erro("错误：不能选择空字节文件");
                                                  return;
                                        }
                                        if (this.configs.HTML5.canvas) {
                                                  if (files.size > this.configs.HTML5FilesSize * 1024 * 1024) {
                                                            this.DelCreateUpfile();
                                                            alert("错误：HTML5上传不能大于" + this.configs.HTML5FilesSize + "M");
                                                            return;
                                                  }
                                        } else if (files.size > this.configs.FilesSize * 1024 * 1024) {
                                                  this.DelCreateUpfile();
                                                  alert("错误：文件不能大于" + this.configs.FilesSize + "M");
                                                  return;
                                        }
                              }
							  var  this_ =this;
                              typeof this.configs.preced === "function"
							   && this.configs.preced(function(){
								           if (this_.configs.HTML5.canvas) {
                                        var reader = new FileReader();
                                        reader.onload = function() {
                                                  this_.DelCreateUpfile();
                                                  this_.configs.HTML5.zipImg(this.result, this_.configs.HTML5ZIP, type, function(DataUrl) {
               ...
               // alert(1977)
                    ...
                    try{
                         ...
                         this.upfile.onclick = this.upfileclick = function() {
                              var $input = $("#selectImage").find('input');
                              typeof this_.configs.fileClick === "function" && this_.configs.fileClick();
                              // alert(1987)
                         };
                    }catch(e){
                         // alert(1988)
                    ...
                    try {
                         // alert(1993)
                         document.getElementById('inp_subt').onchange = function(){
                              // alert(1995)
                              // console.log(this_.upfile.onchange)
